...
最近一次：**480 passed**。全是纯逻辑，不开浏览器、不连云，改完代码先跑这个。
...
**`--deep` 不是走过场。** 上面那 11 个红只有它看得见——`./check.sh` 在当前目录跑，
`.env` 就在旁边；`--deep` 是站在「别人 clone 下来会看到什么」的位置上看的。
它抓到那件事，是在代码第一次被推到第二个远端的那天。

`--deep` 会跟 **`origin` 上已推送的内容**比。本地改了还没 push，它必然报差异——
那是它在如实回答「现在 GitHub 上的那份行不行」，不是误报。
