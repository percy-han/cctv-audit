...
| 5 | **后台任务能活过那条线。** 游离出去的 `asyncio` task 实测跑满 1800s（900s cancel 线的两倍），闲着也不降频，换新对话照样查得到 |
| 6 | **`user_id` 是调用者的真实邮箱**，GE 原样透传。多租户按人分单可行 |
...
### ⚠️ **GE 首页的聊天框，不 @ 就不转给我们；@ 了就能到。**

这是控制台这条路上最重要的一条，也是我在这上面判错三次才写对的一条。

**结论（2026-09-03，真人在浏览器里实测）：**

| 客户在哪说话 | 容器收到没 |
| :--- | :--- |
| GE 首页聊天框，**直接打字不 @** | ❌ **零请求**，两种措辞、三次都没到 |
| GE 首页聊天框，**`@门店视频稽核`** | ✅ 每次都到，`hello` / 稽核请求 / `确认` 全部到 |

> **第三次判错的记录，比结论本身更值得留着。**
> 上面这张表的第二行，我一开始写的是「点进 agent 页面里说话」——**那是我编的**。
> 容器日志只看得见「请求到了」，看不见人是怎么让它到的；操作浏览器的是 Percy，
> 我拿自己的推测替了他的操作，还写进了文档当结论。他后来说「不就是让我 @ 那个
> agent 么」，才发现记错了。
>
> **教训和前两次是同一个：分清「我测到的」和「我以为的」。**
> 容器侧的观测只能证明请求到了，证明不了触发方式；触发方式只有操作的人知道。
> 写文档的时候这两类要分开写，别混成一句话。

首页那个框是 GE 自带助手在回话。它自己就把话答了——报文里能看见证据：
后来点进 agent 之后，GE 把之前的对话历史一起传了下来，里面明明白白写着

```
[user]        "hello Percy"
[root_agent]  "Hello"        ← 这句不是我们回的，那一轮容器根本没被调用
```

**把 `description` 写清楚也没用。** 试过把描述改成中文、直接写「凡是用户提到门店监控、
录像、视频稽核……都交给这个 agent」，首页照样不转。
`Agent.description` 的 schema 说明里写着 "might be used by an LLM to automatically
select an agent"——**"might" 是真的 might，实测不触发。**

schema 那边也堵死了：`Assistant` 资源上**根本没有挂 agent 的字段**
（只有 `generationConfig` / `webGroundingType` / `customerPolicy` 这些），
所以「配一下默认助手让它转发」这个旋钮在 API 上不存在。
这和 `streamAssist` 的 `agentsSpec` 指不动路由是同一个现象。

一个还没排除的变量：agent 的 `sharingConfig` 没设，默认 `RESTRICTED`（按 IAM 判），
另一个可选值是 `ALL_USERS`。**没试过它会不会改变首页的行为**——这是改共享范围，
得客户点头才能动。

→ **Phase 4 的露出问题：客户得知道要打一个 `@`。**
比原先以为的「必须点进 agent 页面」轻得多——@ 是聊天软件里人人都会的动作，
一句话就能教会，不用改产品形态。

**@ 出来之后 Prompt chips 是能选的**（2026-09-03 Percy 在浏览器里实测）。
所以「客户点选 SOP 版本、不手写标准」这个设计在首页这条路上成立，
不用退化成让客户自己打字。

剩下**一个**小问题：客户不 @ 直接打字的话，GE 自带助手会把话接走，
而且答得像模像样，客户不一定看得出没走到我们这儿。
Instructions 挡不住这个（那是我们 agent 的提示词，压根没被调用），
只能写进交付说明。

#### 附带一条：**一个项目里可以有好几个 GE 应用，agent 按应用挂**

```bash
curl ".../collections/default_collection/engines"
cctv-audit                                 ← 我用 API 新建的
gemini-enterprise-17875577_1787557788539   ← 项目自带的那套
hr-policies-lab-engine                     ← 别人的
```

`agents.create` 的 parent 是**某一个 engine（应用）**，不是「GE」这个整体，
两边的 agent 列表互相看不见。同一个 reasoningEngine 可以挂到多个应用下，
各自是独立的 agent 资源：

```bash
GE_APP_ID=<某个应用> python deploy/phase0/create_ge_app.py create-agent <reasoningEngine 资源名>
```

**交付时按客户实际打开的那个应用来注册**，别自己另建一个。

> ⚠️ **这条是真的，但它不是上面那个现象的原因。** 我一度拿它去推翻
> 「首页不转发」的结论，改了文档，结果 Percy 从头到尾都在 `cctv-audit` 里——
> 应用一直是对的。**教训：找到一个更平常的解释 ≠ 原结论错了，改之前先复测一遍。**
> 排查时给两个应用里的 agent 起完全不同的名字，一眼就能分清人在哪边。

### GE 会把整段对话历史传下来

`input.request_json` 里除了当前这句 `message`，还有一个 `events` 数组，
装着这段对话之前的每一轮——**包括我们的容器没参与的那些轮**（比如上面那句
GE 自己回的 "Hello"）。事件形状就是 ADK Event（`author` / `content` / `invocation_id` /
`timestamp`）。

对稽核有用：客户可能先在别处说了店名，再点进 agent 补时间段，容器能自己拼起来。

### `user_id` 是真实邮箱，GE 原样透传

同一批日志里的旁证，回答了"多租户怎么分单"：

| 调用方身份 | 容器收到的 `user_id` |
| :--- | :--- |
| 用 `gcloud auth print-access-token`（真人账号） | `admin@percyhan.altostrat.com` |
| 用 ADC（VM 的服务账号） | `gce-automation-sa@study-project-496907.iam.gserviceaccount.com` |

**GE 传下来的就是调用者本人**，不是某个统一的服务身份。
所以"按人查他自己的单"是可行的，也**必须**按人过滤——
不然张三能查到李四的稽核记录。

**浏览器里真人登录走过来，也是同一个值**（2026-09-03 补测）：Percy 在控制台
点进 agent 说话，容器收到 `user_id: admin@percyhan.altostrat.com`、
`session_id: 10420712702533792746`。**不是某个统一的服务身份，是登录的那个人。**
...
#### 那「甩到后台」到底能不能活过 900s —— ✅ **能，实测 1800s 跑完**

上面那条推论（「不 await 就不会被 cancel」）本来只是推的，而整个方案压在它上面，
所以单独验了一次。探针加了两个动作：`后台 N 秒` 起一个
`asyncio.create_task` 的游离任务并**立刻**返回单号，`状态` 读任务表。

```
经 GE 发「后台 1800 秒」 → 1.7 秒就回「好，开始了，单号 a36b2b」
                            （这一轮 GE 早就结束了）
+660s   running   660/1800s
        ↓ 这里故意 11 分钟一个请求都不发，让容器彻底闲着
+1350s  running  1350/1800s   ← 已经越过 900s 那条线
+1800s  done     1800/1800s   ← 跑满，实例没换
```

**三件事一起证明了：**

1. **900s 的 cancel 只掐"长在请求里"的活。** 游离任务完全不受影响，
   1800 秒整整跑完，是那条线的两倍。
2. **闲着的时候也没被降频。** 中间 11 分钟零请求，任务照样推进了 690 秒
   （另测过一段：墙上 301 秒，任务走 300 秒）。原本担心 Cloud Run
   在没有请求时会把 CPU 掐到接近零，**实测没有**。
3. **换一段全新的对话，照样查得到这个任务。**
   起任务用的是 session `161861831096…`，查状态用的是全新 session
   `17956608235409336570`，一样报出 `a36b2b: done 1800/1800s`——
   因为任务表**不挂在对话上**。这正是客户"我刚才那个查得咋样了"要的行为。

⚠️ **两条别读过头：**

- 这次全程一台实例（探针 `min_instances=1`、`max_instances=2`，没触发扩容）。
  任务表还在**内存**里。真实环境多实例时，起任务和查状态很可能落在不同容器上，
  **该外置还是要外置**——这次证明的是"后台任务能活"，不是"内存够用"。
- "闲着不降频"是在 `min_instances=1` 下测的。设成 0 就是另一回事了。

`_JOB_TASKS` 那个集合是必须的：`asyncio` 对运行中的 task 只持弱引用，
没人拿着就可能被 GC 掉——那种死法看起来和"平台把它杀了"一模一样，
会把排障带到完全错误的方向。
...
**真人在浏览器里也走通了**（2026-09-03 补测，不再只是 API 直连）。
Percy 在控制台点进 agent，同一段对话里连着三轮，容器全收到，`session_id` 一路不变：

```
14006.5s  verb=ask      sess=10420712702533792746  帮我稽核一下 XX 店 昨天 14:00 到 15:00
          →  找到视频了：Plan B 录屏，14:00-15:00 时间段够。确认开始稽核吗？
14073.8s  verb=confirm  sess=10420712702533792746  确认
```

**「先探测、回给用户确认、确认了才跑」这个交互形态，在真实界面里是成立的。**
